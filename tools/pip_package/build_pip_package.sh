...
# check arguments
if [ $# -ne 4 ]; then
    echo "Usage: bash build_pip_package.sh <develop|release> <upbge-version> <source-dir> <upbge-dir>"
    exit 1
fi
...
if [ ${target} = "release" ]; then
    ...
    # generate fake bge module
    fake_module_dir="out"
    ver=v${version}
    bash ${SCRIPT_DIR}/../../src/gen_module.sh ${CURRENT_DIR}/${source_dir} ${CURRENT_DIR}/${upbge_dir} ${UPBGE_TAG_NAME[${ver}]} ${fake_module_dir} ${version}
    zip_dir="fake_bge_modules_${version}-${release_version}"
    cp -r ${fake_module_dir} ${zip_dir}
    zip_file_name="fake_bge_modules_${version}-${release_version}.zip"
    ...
    rm README.md
    rm -rf fake_bge_module*.egg-info/ dist/ build/
    ls -R .
    ...
    fake_module_dir="out"
    ver=v${version}
    bash ${SCRIPT_DIR}/../../src/gen_module.sh ${CURRENT_DIR}/${source_dir} ${CURRENT_DIR}/${upbge_dir} ${UPBGE_TAG_NAME[${ver}]} ${fake_module_dir}
    zip_dir="fake_bge_modules_${version}-${release_version}"
    cp -r ${fake_module_dir} ${zip_dir}
    zip_file_name="fake_bge_modules_${version}-${release_version}.zip"
